...
from SmartLights.traffic.road import Lane, Intersection, Endpoint
from SmartLights.traffic.car import Car
...
ENDPOINT_TRIANGLES = [
    (CENTER, (0,0), (SIZE,0)),
    (CENTER, (SIZE,0), (SIZE, SIZE)),
    (CENTER, (SIZE, SIZE), (0, SIZE)),
    (CENTER, (0, SIZE), (0, 0))
]
...
def intersection_popup() -> None:
    ...
    for tri in INTERSECTION_TRIANGLES:
...
def endpoint_popup() -> None:
    cur = get_item_cell()
    sim[cur[0]][cur[1]].pos = Position(0, 0)
    sim[cur[0]][cur[1]].draw("Endpoint")
    for tri in ENDPOINT_TRIANGLES:
        dpg.draw_triangle(*tri, parent="Endpoint", color=(255,255,0,100))
    if isinstance(sim[cur[0]][cur[1]], Intersection):
        dpg.configure_item("Endpoint", show=True, pos=dpg.get_item_pos("context_popup"))

def click_handler(sender: int, app_data: int, user_data: int) -> None:
    pos = dpg.get_mouse_pos()
    cur = get_item_cell()
    if dpg.is_item_hovered("Intersection"):
        for t in range(len(INTERSECTION_TRIANGLES)):
            if point_in_triangle(pos, *INTERSECTION_TRIANGLES[t]):
                if user_data == 0:
                    ...
                    for l in range(len(sim[cur[0]][cur[1]].children)):
                        ...
                        if isinstance(c, Lane) and c.side == int(t/2) and c.isEntering == (t+1)%2:
                            ...
                            break
                draw_update(draw_list)
    elif dpg.is_item_hovered("Endpoint"):
        for t in range(len(ENDPOINT_TRIANGLES)):
            if point_in_triangle(pos, *ENDPOINT_TRIANGLES[t]):
                if user_data == 0:
                    sim[cur[0]][cur[1]].add_endpoint(Endpoint(Position(cur[0]*SIZE, cur[1]*SIZE), t))
                    sim[cur[0]][cur[1]].pos = Position(0, 0)
                    sim[cur[0]][cur[1]].draw("Endpoint")
                    for tri in ENDPOINT_TRIANGLES:
                        dpg.draw_triangle(*tri, parent="Endpoint", color=(255,255,0,100))
                elif user_data == 1:
                    sim[cur[0]][cur[1]].rm_endpoint(t)
                    sim[cur[0]][cur[1]].pos = Position(0, 0)
                    for e in sim[cur[0]][cur[1]].endpoints:
                        e.pos = Position(0, 0)
                    sim[cur[0]][cur[1]].draw("Endpoint")
                    for tri in ENDPOINT_TRIANGLES:
                        dpg.draw_triangle(*tri, parent="Endpoint", color=(255,255,0,100))
...
with dpg.window(label="Modify an Intersection", tag="Intersection", show=False, modal=True,
    ...
    pass
with dpg.window(label="Modify an Endpoint", tag="Endpoint", show=False, modal=True,
                no_title_bar=False, width=SIZE+20, height=SIZE+50):
    pass

with dpg.window(width=WINDOWWIDTH, height=WINDOWHEIGHT, pos=(0, 0),
                label="Simulation", no_move=True, no_collapse=True, no_close=True, no_title_bar=True):
    global draw_list
    draw_list = dpg.add_drawlist(width=WINDOWWIDTH, height=WINDOWHEIGHT, pos=(0,0))
    draw_update(draw_list)
    ...
    with dpg.popup(parent=draw_list, mousebutton=dpg.mvMouseButton_Left, tag="context_popup"):
        ...
        dpg.add_menu_item(label="Modify Endpoints",     callback=endpoint_popup)
        dpg.add_menu_item(label="Delete Intersection",  callback=delete_cell)
